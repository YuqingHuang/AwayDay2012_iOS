...
- (void)postUserShare2Server {
    ...
    AFHTTPClient *postClient = [[AFHTTPClient alloc] init];
    NSMutableURLRequest *urlRequest;
    urlRequest = [postClient multipartFormRequestWithMethod:@"POST"
                                                       path:kServiceUserShare
                                                 parameters:nil
                                  constructingBodyWithBlock:^(id <AFMultipartFormData> formData) {
                                      [formData appendPartWithFormData:[paramString dataUsingEncoding:NSUTF8StringEncoding] name:nil];
                                  }];
    ...
    AFJSONRequestOperation *operation = [AFJSONRequestOperation JSONRequestOperationWithRequest:urlRequest
                                                                                        ...
                                                                                        failure:^(NSURLRequest *request, NSHTTPURLResponse *response, NSError *error, id JSON) {
                                                                                            NSLog(@"fail response:%@", request.HTTPBodyStream);
                                                                                            [AppHelper removeInfoView:self.view];
...
//        [param setObject:[AppHelper base64DecodeImage:self.userImage] forKey:kShareImageKey];
    ...
    NSURL *url = [NSURL URLWithString:kServiceUserShare];
